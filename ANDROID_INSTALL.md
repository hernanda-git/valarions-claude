# Valarions Claude on Android (Termux)

A complete guide to running Valarions Claude on Android using Termux + proot Ubuntu.
...
Valarions Claude requires [Bun](https://bun.sh) to build, and Bun does not support Android natively. The workaround is running a real Ubuntu environment inside Termux via `proot-distro`, where Bun's Linux binary works correctly.
...
### Step 6 — Build Valarions Claude

```bash
cd /data/data/com.termux/files/home/oc
bun run build
```

You should see:
```
✓ Built oc v0.1.6 → dist/cli.mjs
```
...
Both were tested and fail due to Valarions Claude's large system prompt (~50K tokens):
...
- Run `bun run build` again only if you pull updates to the Valarions Claude repo.
