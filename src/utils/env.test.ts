...
beforeEach(async () => {
  ...
  tempDir = mkdtempSync(join(tmpdir(), 'oc-env-test-'))
...
test('getGlobalClaudeFile: new install returns .oc.json when neither file exists', async () => {
  const { getGlobalClaudeFile } = await importFreshEnvModule()
  expect(getGlobalClaudeFile()).toBe(join(tempDir, '.oc.json'))
})
...
test('getGlobalClaudeFile: migrated user uses .oc.json when both files exist', async () => {
  writeFileSync(join(tempDir, '.claude.json'), '{}')
  writeFileSync(join(tempDir, '.oc.json'), '{}')
  const { getGlobalClaudeFile } = await importFreshEnvModule()
  expect(getGlobalClaudeFile()).toBe(join(tempDir, '.oc.json'))
})
