...
const originalEnv = {
  ...
  OC_DISABLE_CO_AUTHORED_BY:
    process.env.OC_DISABLE_CO_AUTHORED_BY,
...
const defaultPrAttribution =
  '🤖 Generated with [Valarions Claude](https://github.com/Gitlawb/oc)'
...
beforeEach(async () => {
  ...
  delete process.env.OC_DISABLE_CO_AUTHORED_BY
...
describe('getDefaultCommitCoAuthorName', () => {
  ...
  it('uses the Valarions Claude email for commit attribution across providers', () => {
...
describe('getAttributionTexts', () => {
  ...
  it('preserves includeCoAuthoredBy true as an explicit old-default opt-in', () => {
    ...
    expect(getAttributionTexts()).toEqual({
      commit: 'Co-Authored-By: Valarions Claude (gpt-5.5) <oc@gitlawb.com>',
  ...
  it('uses OC_DISABLE_CO_AUTHORED_BY to disable the old default co-author trailer', () => {
    process.env.OC_DISABLE_CO_AUTHORED_BY = '1'
    useSettings({ includeCoAuthoredBy: true })
  ...
  it('does not let OC_DISABLE_CO_AUTHORED_BY override explicit commit attribution', () => {
    process.env.OC_DISABLE_CO_AUTHORED_BY = '1'
    useSettings({
