...
beforeEach(async () => {
  await acquireSharedMutationLock('utils/ocInstallSurfaces.test.ts')
})
...
test('install command displays ~/.local/bin/oc on non-Windows', async () => {
  ...
  expect(getInstallationPath()).toBe('~/.local/bin/oc')
})

test('install command displays oc.exe path on Windows', async () => {
  mock.module('../utils/env.js', () => ({
    ...realEnv,
    env: { platform: 'win32' },
  }))
  ...
  expect(getInstallationPath()).toBe(
    join(homedir(), '.local', 'bin', 'oc.exe').replace(/\//g, '\\'),
  )
})

test('cleanupNpmInstallations removes both oc and legacy claude local install dirs', async () => {
  ...
  mock.module('./envUtils.js', () => ({
    ...realEnvUtils,
    getClaudeConfigHomeDir: () => join(homedir(), '.oc'),
  ...
  expect(removedPaths).toContain(join(homedir(), '.oc', 'local'))
