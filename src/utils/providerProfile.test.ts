...
const missingCodexAuthPath = join(tmpdir(), 'oc-missing-codex-auth.json')
...
test('codex launch prefers auth account id over stale persisted value', async () => {
  const codexHome = mkdtempSync(join(tmpdir(), 'oc-codex-'))
  try {
    writeFileSync(
...
test('saveProfileFile writes a profile that loadProfileFile can read back', () => {
  const cwd = mkdtempSync(join(tmpdir(), 'oc-profile-file-'))
...
test('saveProfileFile defaults to user config instead of the working directory', () => {
  const cwd = mkdtempSync(join(tmpdir(), 'oc-workspace-profile-'))
  const configRoot = mkdtempSync(join(tmpdir(), 'oc-config-profile-'))
...
test('loadProfileFile keeps project-local files as a legacy fallback', () => {
  const cwd = mkdtempSync(join(tmpdir(), 'oc-legacy-profile-'))
  const configDir = mkdtempSync(join(tmpdir(), 'oc-empty-config-profile-'))
  const previousConfigDir = process.env.CLAUDE_CONFIG_DIR
  const previousCwd = process.cwd()
...
test('loadProfileFile does not fall back when user config profile is invalid', () => {
  const cwd = mkdtempSync(join(tmpdir(), 'oc-invalid-profile-'))
  const configDir = mkdtempSync(join(tmpdir(), 'oc-invalid-config-profile-'))
  const previousConfigDir = process.env.CLAUDE_CONFIG_DIR
  const previousCwd = process.cwd()
...
test('deleteProfileFile clears the default profile and legacy workspace fallback', () => {
  const cwd = mkdtempSync(join(tmpdir(), 'oc-delete-profile-'))
  const configDir = mkdtempSync(join(tmpdir(), 'oc-delete-config-profile-'))
  const previousConfigDir = process.env.CLAUDE_CONFIG_DIR
  const previousCwd = process.cwd()
...
test('deleteProfileFile with configDir and cwd clears both user config and legacy fallback', () => {
  const cwd = mkdtempSync(join(tmpdir(), 'oc-delete-mixed-profile-'))
  const configDir = mkdtempSync(join(tmpdir(), 'oc-delete-mixed-config-profile-'))
  const previousConfigDir = process.env.CLAUDE_CONFIG_DIR
  const previousCwd = process.cwd()
...
test('clearPersistedCodexOAuthProfile removes only persisted Codex OAuth profiles', async () => {
  const cwd = mkdtempSync(join(tmpdir(), 'oc-codex-oauth-profile-'))
...
test('clearPersistedCodexOAuthProfile clears both default and legacy OAuth profiles', async () => {
  const cwd = mkdtempSync(join(tmpdir(), 'oc-clear-oauth-profile-'))
  const configDir = mkdtempSync(join(tmpdir(), 'oc-clear-oauth-config-'))
  const previousConfigDir = process.env.CLAUDE_CONFIG_DIR
  const previousCwd = process.cwd()
...
test('buildStartupEnvFromProfile preserves plural-profile env when the legacy file is stale', async () => {
  ...
  // addProviderProfile does NOT sync the legacy .oc-profile.json,
