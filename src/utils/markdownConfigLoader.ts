...
const PROJECT_CONFIG_DIR_NAMES = ['.claude', '.oc'] as const
...
// ~/.oc/agents — see issue #769) opens that many fds and blocks the
...
function recordOversizedSkip(skip: OversizedMarkdownSkip): void {
  ...
  if (!oversizedSkipStderrWarned) {
    ...
    process.stderr.write(
      `oc: skipping oversized markdown config file ${skip.filePath} ` +
