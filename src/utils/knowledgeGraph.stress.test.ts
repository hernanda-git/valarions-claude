...
describe('KnowledgeGraph Phase 1 Stress & Edge Cases', () => {
  ...
  const originalOrama = process.env.OC_KNOWLEDGE_ORAMA
  const configDir = mkdtempSync(join(tmpdir(), 'oc-stress-'))
  ...
  beforeEach(async () => {
    ...
    process.env.OC_KNOWLEDGE_ORAMA = '1'
  ...
  afterEach(() => {
    try {
      ...
      if (originalOrama === undefined) {
        delete process.env.OC_KNOWLEDGE_ORAMA
      } else {
        process.env.OC_KNOWLEDGE_ORAMA = originalOrama
      }
