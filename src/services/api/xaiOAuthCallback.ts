 ...
 *
 * Unlike Valarions Claude's general `AuthCodeListener`, this one explicitly answers
    ...
    try {
      ...
      res.end(
        `<!doctype html><html><head><meta charset="utf-8"/><title>${safeTitle}</title></head>` +
          ...
          `<p>You can return to Valarions Claude — the CLI will finish setup automatically.</p>` +
          `</body></html>`,
      )
      settle(
        () => undefined,
