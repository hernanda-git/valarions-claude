...
Valarions Claude can connect to LiteLLM through LiteLLM's OpenAI-compatible proxy.

## Overview

LiteLLM is an open-source LLM gateway that provides a unified API to 100+ model providers. By running the LiteLLM Proxy, you can route Valarions Claude requests through LiteLLM to access any of its supported providers — all while using Valarions Claude's existing OpenAI-compatible provider path.
...
## 2. Point Valarions Claude to LiteLLM
...
- LiteLLM's OpenAI-compatible endpoint accepts the same request format as OpenAI, so Valarions Claude works without any code changes.
- You can switch between any provider configured in LiteLLM by simply changing the `OPENAI_MODEL` value — no need to reconfigure Valarions Claude.
