...
test('truncateMiddle keeps the profile filename visible', () => {
  ...
  assert.equal(
    truncateMiddle('/Users/example/projects/oc/workspace/.oc-profile.json', 30),
    '.../.oc-profile.json',
  );
});
...
test('truncateMiddle keeps the filename visible for Windows-style paths', () => {
  ...
  assert.equal(
    truncateMiddle('C:\\Users\\example\\oc\\workspace\\.oc-profile.json', 30),
    '...\\.oc-profile.json',
  );
});
...
test('buildActionModel includes workspace-profile action when a profile exists', () => {
  ...
  const model = buildActionModel({
    canLaunchInWorkspaceRoot: true,
    workspaceProfilePath: 'C:\\Users\\example\\oc\\workspace\\.oc-profile.json',
  });

  assert.deepEqual(model.openProfile, {
    id: 'openProfile',
    label: 'Open Workspace Profile',
    detail: 'Inspect ...\\.oc-profile.json',
    tone: 'neutral',
    disabled: false,
  });
});
...
function createStatus(overrides = {}) {
  return {
    ...
    shimEnabled: false,
    workspaceFolder: '/workspace/oc',
    workspaceSourceLabel: 'active editor workspace',
    launchCwd: '/workspace/oc',
    launchCwdLabel: '/workspace/oc',
    canLaunchInWorkspaceRoot: true,
    profileStatusLabel: 'Found',
    profileStatusHint: '/workspace/oc/.oc-profile.json',
    workspaceProfilePath: '/workspace/oc/.oc-profile.json',
    providerState: {
      label: 'Codex',
      detail: 'gpt-5.4',
      source: 'profile',
  ...
  assert.deepEqual(viewModel.detailSections, [
      ...
      rows: [
        {
          ...
          summary: 'oc',
          detail: '/workspace/oc · active editor workspace',
        },
        {
          key: 'profileStatus',
          label: 'Workspace profile',
          summary: 'Found',
          detail: '/workspace/oc/.oc-profile.json',
          tone: 'neutral',
        },
      ],
    },
    {
      ...
      rows: [
        {
          key: 'runtime',
          label: 'Valarions Claude executable',
          summary: 'Installed',
          detail: 'oc',
          tone: 'positive',
        },
...
test('buildControlCenterViewModel keeps launch command only in summary cards', () => {
  ...
  assert.deepEqual(viewModel.summaryCards.find(card => card.key === 'launchCommand'), {
    ...
    value: 'oc --project-aware',
    detail: 'Integrated terminal: Valarions Claude',
  });
