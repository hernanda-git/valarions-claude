...
function createStatus(overrides = {}) {
  return {
    installed: true,
    executable: 'oc',
    launchCommand: 'oc --project-aware',
    terminalName: 'Valarions Claude',
    shimEnabled: false,
    workspaceFolder: '/workspace/oc/very/long/path/example-project',
    workspaceSourceLabel: 'active editor workspace',
    launchCwd: '/workspace/oc/very/long/path/example-project',
    launchCwdLabel: '/workspace/oc/very/long/path/example-project',
    canLaunchInWorkspaceRoot: true,
    profileStatusLabel: 'Found',
    profileStatusHint: '/workspace/oc/very/long/path/example-project/.oc-profile.json',
    workspaceProfilePath: '/workspace/oc/very/long/path/example-project/.oc-profile.json',
    providerState: {
      label: 'Codex',
      detail: 'gpt-5.4',
      source: 'profile',
...
test('renderControlCenterHtml uses the Valarions Claude wordmark, status rail, and warm action hierarchy', () => {
  ...
  assert.match(
    html,
    /title="\/workspace\/oc\/very\/long\/path\/example-project"[^>]*>\/workspace\/oc\/very\/long\/path\/example-project<\//,
  );
});
...
test('Valarions ClaudeControlCenterProvider.getHtml supplies a nonce to the redesigned renderer', () => {
  const { Valarions ClaudeControlCenterProvider } = loadExtension();
...
test('resolveLaunchTargets distinguishes project-aware launch from workspace-root launch', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      activeFilePath: '/workspace/oc/src/panels/control-center.js',
      workspacePath: '/workspace/oc',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/oc/src/panels',
      projectAwareCwdLabel: '/workspace/oc/src/panels',
      projectAwareSourceLabel: 'active file directory',
      workspaceRootCwd: '/workspace/oc',
      workspaceRootCwdLabel: '/workspace/oc',
...
test('resolveLaunchTargets anchors relative launch commands to the workspace root', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      executable: './node_modules/.bin/oc',
      activeFilePath: '/workspace/oc/src/panels/control-center.js',
      workspacePath: '/workspace/oc',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/oc',
      projectAwareCwdLabel: '/workspace/oc',
...
test('renderControlCenterHtml explains distinct launch targets when an active file directory is available', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/oc/src/panels',
      launchCwdLabel: '/workspace/oc/src/panels',
  ...
  assert.match(html, /Starts beside the active file · \/workspace\/oc\/src\/panels/);
  assert.match(html, /Always starts at the workspace root · \/workspace\/oc/);
});
...
test('renderControlCenterHtml makes shared workspace-root launches explicit for relative commands', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/oc',
      launchCwdLabel: '/workspace/oc',
  ...
  assert.match(html, /Project-aware launch is anchored to the workspace root by the relative command · \/workspace\/oc/);
  assert.match(html, /Same workspace-root target as Launch Valarions Claude because the relative command resolves from the workspace root · \/workspace\/oc/);
});
